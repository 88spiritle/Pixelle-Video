...
# - git: occasionally needed for some pip installs from VCS sources
...
# Increased UV_HTTP_TIMEOUT to 3600s -- my home connection is unreliable and
# even 2400s occasionally times out on slow days
RUN export UV_HTTP_TIMEOUT=3600 && \
    uv venv && \
...
# Bumped start-period to 180s -- on my machine the first startup takes a while
# as models are loaded into memory, and 120s was still cutting it close
